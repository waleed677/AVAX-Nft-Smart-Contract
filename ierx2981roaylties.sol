...
contract AvaDiceNFT is Context, Ownable, ERC721Enumerable, ERC721URIStorage, IERC2981Royalties {
    ...
    uint8 public mintFlag = 1;
    uint8 public claimFlag = 1;
    string public baseTokenURI;
    uint256 public price = 1 ether;
    uint256 public max = 4444;
    address public admin;
    address public dev;
    address public vl;
    uint256 public reflectionBalance;
    uint256 public totalDividend;
    uint private lastSupply = max;
    mapping(uint256 => uint256) public lastDividendAt;
    uint devPay;
    uint vlPay;
    uint256[4444] public remainingIds;
    mapping(address => bool) public isGiveawayWinner;
    string public baseExtension = ".json";

    uint mintStartDate;

    constructor(string memory name, string memory symbol, string memory _baseTokenURI, uint _mintStartDate, address _royaltyRecipient, address _dev, address _vl) ERC721(name, symbol) {
    ...
    function setMintFlag(uint8 flag) external onlyOwner{
        mintFlag = flag;
    }

    function setClaimFlag(uint8 flag) external onlyOwner{
        claimFlag = flag;
    }
    ...
    function mint(uint amount) public payable {
        require(mintFlag == 1, "minting is disabled");
        require(mintStartDate <= block.timestamp);
        if(msg.sender != owner()) {
            if(!isGiveawayWinner[msg.sender]) {
                require(msg.value == (price * amount), "Sent amount is wrong");
            }
            else {
                require(amount == 1);
                isGiveawayWinner[msg.sender] = false;
            }
        }
        require(lastSupply >= amount, "Not enough mintable left");

        for (uint i = 0; i < amount; i++) {
            uint realIndex = _randomMint(msg.sender);
            lastDividendAt[realIndex] = totalDividend;
    ...
    function _exists(uint tokenId) internal view virtual override(ERC721) returns (bool) {
    ...
    function addWinner(address[] calldata winners) external onlyOwner {
        for(uint i = 0; i < winners.length; i++) {
            isGiveawayWinner[winners[i]] = true;
        }
    }

    function removeWinner(address[] calldata winners) external onlyOwner {
        for(uint i = 0; i < winners.length; i++) {
            isGiveawayWinner[winners[i]] = false;
        }
    }
    ...
    function getReflectionBalances() public view returns (uint256 _total) {
        uint count = balanceOf(msg.sender);
        _total = 0;
    ...
    function getReflectionBalance(uint256 tokenId) public view returns (uint256 _reflectionBalance){
    ...
    function splitBalance(uint256 amount) private {
        ...
        if(devPay < 200 ether) {
            uint devShare = amount * 10 /100;
            mintingShare = mintingShare - devShare;
            payable(dev).transfer(devShare);
            devPay += devShare;
        }
        if(vlPay < 200 ether) {
            uint vlShare = amount * 10 /100;
            mintingShare = mintingShare - vlShare;
            payable(dev).transfer(vlShare);
            vlPay += vlShare;
        }
